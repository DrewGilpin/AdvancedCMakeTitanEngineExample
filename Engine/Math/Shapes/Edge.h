...
// squared distance between point and a straight line, 'line_dir'=line direction (doesn't have to be normalized, however it's slower than 'Dist2PointLine')
Flt Dist2PointLineN(C Vec  &point,                   C Vec   &line_dir); // this function assumes that line position is Vec(0,0,0)
Flt Dist2PointLineN(C Vec  &point, C Vec  &line_pos, C Vec   &line_dir);
Dbl Dist2PointLineN(C VecD &point, C Vec  &line_pos, C Vec   &line_dir);
Dbl Dist2PointLineN(C VecD &point, C VecD &line_pos, C VecD  &line_dir);
