...
struct MeshPart // Mesh Base + Mesh Render
{
   ...
   // set
#if EE_PRIVATE
   MeshPart& remapMaterials(Byte new_index[5]);
   MeshPart& setUMM        ();
#endif
   ...
   MeshPart& setRender    (VTX_COMPRESS compress=VTX_COMPRESS_DEFAULT, Int lod_index=0); // set rendering version, convert 'MeshBase'   to 'MeshRender', 'lod_index'=index of the LOD in the mesh (used to determine quality of the shader)
 ...
 C MaterialPtr& multiMaterial(Int i)C; // get i-th multi material
   ...
   // UV transform
   MeshPart& uvMove  (C Vec2 &move , Byte uv_index=0); // move   UVs
   MeshPart& uvScale (C Vec2 &scale, Byte uv_index=0); // scale  UVs
   MeshPart& uvRotate(  Flt   angle, Byte uv_index=0); // rotate UVs
   ...
   MeshPart& setRandomLeafBending(MeshLod &src      ); // this method will set    unique bending for different leafs (this will only affect mesh parts which have leaf-based material), 'src'=source LOD that this part was created from
   ...
   // draw
      ...
      // draw overlay
      void drawOverlayCurMatrix(C Image &image, C Color &color=WHITE)C; // draw with current matrix as overlay using 'image' and 'color', this can be called only in RM_OVERLAY, doesn't use automatic Frustum culling
      ...
      void drawBoneHighlightCurMatrix(Int bone)C; // draw with current matrix, 'bone'=bone index to highlight
...
private:
   ...
   struct Variation
   {
   ...
   #endif
      {
         mutable Int first_opaque_instance, first_shadow_instance; // keep 'first_opaque_instance' as first member, because it's used most often, have to keep 2 separate, because in forward renderer we queue opaque draw calls, but before we draw them, we process shadows first
      };
      ...
      void unlinkOpaque()C {first_opaque_instance=-1;}
      void unlinkShadow()C {first_shadow_instance=-1;}
      void unlink      ()C {unlinkOpaque(); unlinkShadow();}
      Bool drawn       ()C {return first_opaque_instance>=0 || first_shadow_instance>=0;}
   ...
   MaterialPtr     _materials[4]; // extra multi-materials in addition to #0 'material'
#if EE_PRIVATE
