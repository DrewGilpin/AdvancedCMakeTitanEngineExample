...
enum MESH_FLAG : ULong // Mesh Flag
{
   ...
   VTX_POS1    =1ull<< 4, // vertex position 1
   VTX_NRM     =1ull<< 1, // vertex normal
   VTX_TAN     =1ull<< 2, // vertex tangent
   VTX_BIN     =1ull<< 3, // vertex binormal
   VTX_UV      =1ull<< 5, // vertex texture UV 0
   VTX_UV1     =1ull<< 6, // vertex texture UV 1
   VTX_UV2     =1ull<< 7, // vertex texture UV 2
   VTX_UV3     =1ull<< 8, // vertex texture UV 3
   ...
   VTX_UV_ALL     =VTX_UV    |VTX_UV1|VTX_UV2|VTX_UV3,
   VTX_UV_SEC     =           VTX_UV1|VTX_UV2|VTX_UV3, // secondary
   ...
   FACE_ID      =TRI_ID      |QUAD_ID      ,
    ...
    VTX_ALL= VTX_POS|VTX_POS1|VTX_NRM_TAN_BIN|VTX_UV_ALL|VTX_COLOR|VTX_MATERIAL|VTX_MATRIX|VTX_BLEND|VTX_SIZE|VTX_DUP|VTX_FLAG,
...
#if EE_PRIVATE
   VTX_MSHR=VTX_POS|VTX_POS1|VTX_NRM_TAN_BIN|VTX_UV_ALL|VTX_COLOR|VTX_MATERIAL|VTX_MATRIX|VTX_BLEND|VTX_SIZE, // vertex values which can be stored in MeshRender
#endif
};
...
/******************************************************************************/
#if EE_PRIVATE
Vec5  DecodeMaterial(C VecB4 &mtrl_weight);
VecB4 EncodeMaterial(C VecB5 &mtrl_weight);
VecB4 EncodeMaterial(C VecI5 &mtrl_weight);
VecB4 EncodeMaterial(C Vec5  &mtrl_weight);
...
struct  VtxFull // Vertex containing all possible data
{
   ...
   VtxFull& mul(C Matrix &matrix, C Matrix3 &matrix3); // transform vertex by matrixes ('pos pos1' by 'matrix', and 'nrm tan bin' by 'matrix3')
};
/******************************************************************************/
struct MeshVtxs // Mesh Vertexes
{
 ...
 C Vec    * pos     ()C {return _pos     ;}   C Vec    & pos     (Int i)C {DEBUG_RANGE_ASSERT(i, _elms); return _pos     [i];} // get i-th vertex position
   Vec    * pos1    ()  {return _pos1    ;}     Vec    & pos1    (Int i)  {DEBUG_RANGE_ASSERT(i, _elms); return _pos1    [i];} // get i-th vertex position 1
 C Vec    * pos1    ()C {return _pos1    ;}   C Vec    & pos1    (Int i)C {DEBUG_RANGE_ASSERT(i, _elms); return _pos1    [i];} // get i-th vertex position 1
 ...
 C Vec    * bin     ()C {return _bin     ;}   C Vec    & bin     (Int i)C {DEBUG_RANGE_ASSERT(i, _elms); return _bin     [i];} // get i-th vertex binormal
   Vec2   * uv      ()  {return _uv      ;}     Vec2   & uv      (Int i)  {DEBUG_RANGE_ASSERT(i, _elms); return _uv      [i];} // get i-th vertex texture UV 0
 C Vec2   * uv      ()C {return _uv      ;}   C Vec2   & uv      (Int i)C {DEBUG_RANGE_ASSERT(i, _elms); return _uv      [i];} // get i-th vertex texture UV 0
   Vec2   * uv1     ()  {return _uv1     ;}     Vec2   & uv1     (Int i)  {DEBUG_RANGE_ASSERT(i, _elms); return _uv1     [i];} // get i-th vertex texture UV 1
 C Vec2   * uv1     ()C {return _uv1     ;}   C Vec2   & uv1     (Int i)C {DEBUG_RANGE_ASSERT(i, _elms); return _uv1     [i];} // get i-th vertex texture UV 1
   Vec2   * uv2     ()  {return _uv2     ;}     Vec2   & uv2     (Int i)  {DEBUG_RANGE_ASSERT(i, _elms); return _uv2     [i];} // get i-th vertex texture UV 2
 C Vec2   * uv2     ()C {return _uv2     ;}   C Vec2   & uv2     (Int i)C {DEBUG_RANGE_ASSERT(i, _elms); return _uv2     [i];} // get i-th vertex texture UV 2
   Vec2   * uv3     ()  {return _uv3     ;}     Vec2   & uv3     (Int i)  {DEBUG_RANGE_ASSERT(i, _elms); return _uv3     [i];} // get i-th vertex texture UV 3
 C Vec2   * uv3     ()C {return _uv3     ;}   C Vec2   & uv3     (Int i)C {DEBUG_RANGE_ASSERT(i, _elms); return _uv3     [i];} // get i-th vertex texture UV 3
...
private:
#endif
   Int _elms; Vec *_pos, *_pos1, *_nrm, *_tan, *_bin; Vec2 *_uv, *_uv1, *_uv2, *_uv3; Color *_color; VecB4 *_material; VtxBone *_matrix; VecB4 *_blend; Flt *_size; Byte *_flag; Int *_dup;
...
struct MeshBase // Mesh Base (the most low level software mesh, contains : Vertexes + Edges + Triangles + Quads)
{
...
#if EE_PRIVATE
   MeshBase& create    (C MeshBase  *src[], Int elms, MESH_FLAG flag_and=MESH_ALL        , Bool set_face_id_from_part_index=false                              ); // create from 'src' array, 'flag_and'=data to copy, 'set_face_id_from_part_index'=if set face 'id' members to the index of the original MeshPart that they were created from
   Bool      createVtx (C VtxBuf &vb, MESH_FLAG flag, VTX_COMPRESS compress, MESH_FLAG flag_and=MESH_ALL                                        ) ; // create vertexes from vertex buffer
   ...
   MeshBase& create      (C Ball    &ball   , MESH_FLAG flag=MESH_NONE, Int resolution=-1                    ); // create mesh as 3D ball in cube      mode, 'flag'=data to set in the mesh
   MeshBase& create2     (C Ball    &ball   , MESH_FLAG flag=MESH_NONE, Int resolution=-1, Int resolution2=-1); // create mesh as 3D ball in spherical mode, 'flag'=data to set in the mesh
   MeshBase& createIco   (C Ball    &ball   , MESH_FLAG flag=MESH_NONE, Int resolution=-1                    ); // create mesh as 3D ball in icosphere mode, 'flag'=data to set in the mesh
   ...
   MeshBase& setVtxDupEx   (MESH_FLAG flag=MESH_NONE, Flt pos_eps=EPS, Flt nrm_cos=EPS_COL_COS, Flt tan_cos=EPS_TAN_COS, Flt bin_cos=EPS_BIN_COS, Bool uv_wrap=false, Bool smooth_groups_in_vtx_material=false);
#endif
   ...
   // UV transform
   MeshBase& uvMove  (C Vec2 &move , Byte uv_index=0); // move   UVs
   MeshBase& uvScale (C Vec2 &scale, Byte uv_index=0); // scale  UVs
   MeshBase& uvRotate(  Flt   angle, Byte uv_index=0); // rotate UVs

   // texturize
   MeshBase& uvMap  (  Flt     scale=1, Byte uv_index=0); // map UVs according to vertex XY position and scale
   MeshBase& uvMapXZ(  Flt     scale=1, Byte uv_index=0); // map UVs according to vertex XZ position and scale
   MeshBase& uvMap  (C Matrix &matrix , Byte uv_index=0); // map UVs according to matrix
   MeshBase& uvMap  (C Plane  &plane  , Byte uv_index=0); // map UVs according to plane
   MeshBase& uvMap  (C Ball   &ball   , Byte uv_index=0); // map UVs according to ball
   MeshBase& uvMap  (C Tube   &tube   , Byte uv_index=0); // map UVs according to tube
   ...
   // fix
   MeshBase& fixUVOffset  (Byte uv_index=0); // fix texture offset  , this reduces big texture coordinates to small ones increasing texturing quality on low precision video cards
   MeshBase& fixUVWrapping(Byte uv_index=0); // fix texture wrapping, fixes texture coordinates created by spherical/tube mapping (this can add new vertexes to the mesh)
   MeshBase& fixFaceSide  (               ); // fix all faces to be on the same side in case some are reversed
