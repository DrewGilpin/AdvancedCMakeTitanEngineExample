...
struct SkyClass
{
   ...
   SkyClass& orientation(C Matrix3 &orn);   Matrix3 orientation()C {return _orn;} // set/get sky orientation, 'orn' must be normalized, default=MatrixIdentity, affects skyboxes and atmospheric stars
...
#if EE_PRIVATE
   Bool      isActual     ()C {return _is && D.viewFovPerspective();}
   SkyClass& del          ();
   SkyClass& create       ();
   void      draw         ();
   void      setFracMulAdd();
...
private:
   ...
   Sky; // Main Sky
/******************************************************************************/
struct Atmosphere : BallM, BlendObject // BallM.r = total radius of the atmosphere = planet radius + atmosphere height, BallM.pos = world-space position
{
   ...
   Vec rayleigh_scattering; // Rayleigh scattering, 0..Inf

   Atmosphere() {}
   Atmosphere(C VecD &pos, Flt planet_radius, Flt height, Flt light_scale) {T.pos=pos; T.planet_radius=planet_radius; T.height=height; T.r=planet_radius+height; T.light_scale=light_scale; T.fog_reduce=1.0f; T.fog_reduce_dist=height; T.darken=14; T.mie_extinction=0.0016f; T.rayleigh_scattering.set(5.2091275314786692e-06f, 1.2171661977460255e-05f, 2.9715971624658822e-05f);}
   ...
   void draw(); // draw this Atmosphere object, this should be called only in RM_PREPARE mode !! OBJECT MUST REMAIN IN CONSTANT MEMORY ADDRESS UNTIL RENDERING HAS FINISHED !!
#if EE_PRIVATE
