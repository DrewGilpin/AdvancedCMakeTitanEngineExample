...
struct RendererClass // handles rendering
{
 ...
 C VecI2& res ()C {return _res  ;} // get Current Render Target Size   (in pixels)
   ...
   Bool canReadDepth()C;   Bool safeCanReadDepth()C;   Bool canReadDepth1S()C;   Bool hasDepthAttached()C;
...
#if SUPPORT_STENCIL
   Bool hasStencilAttached()C;
#else
   Bool hasStencilAttached()C {return false;}
#endif
#endif

   // get / set
   RendererClass& type            (RENDER_TYPE type);   RENDER_TYPE type             ()C {return SUPPORT_RT_FORWARD ?  _type : RT_DEFERRED;} // set/get Rendering Type, default=RT_DEFERRED, the change is NOT instant, avoid calling real-time
                                                        ...
                                                        Bool            firstPass    ()C {return SUPPORT_RT_FORWARD ?  _first_pass :  true;} // if      currently rendering the first   pass (    the main one) of the Forward Renderer
                                                        Bool        secondaryPass    ()C {return SUPPORT_RT_FORWARD ? !_first_pass : false;} // if      currently rendering a secondary pass (not the main one) of the Forward Renderer
...
#if EE_PRIVATE
   RENDER_TYPE curType()C {return SUPPORT_RT_FORWARD ? _cur_type : RT_DEFERRED;}
   ...
   void detectNaNInf(ImageRTPtr &rt);
...
private:
 ...
 C Memc<ShaderParamChange> *_shader_param_changes;
   ...
   ImageRTPtr            _h0, _h1, _q0, _q1, // <- these members are to be used only temporarily
                         ...
                         _water_col, _water_nrm, _water_refract, _water_ds, _water_lum, _water_spec,
   ...
   Int                   _t_measures[2];
   Dbl                   _t_last_measure;
   Flt                   _t_reflection[2], _t_prepare[2], _t_opaque[2], _t_overlay[2], _t_water[2], _t_light[2], _t_emissive[2], _t_sky[2], _t_edge_detect[2], _t_blend[2], _t_palette[2], _t_behind[2], _t_rays[2], _t_volumetric[2], _t_post_process[2], _t_gpu_wait[2];
...
#else
inline void SetStencilValue(Bool terrain     =false) {}
inline void SetStencilMode (Bool terrain_only=false) {}
#endif
