...
struct GroupMatrix
{
   Int     end; // index in the instances container at which this group ends
   MatrixM cam_matrix, cam_matrix_prev;

   void set(Int end) {T.end=end; cam_matrix=CamMatrix; cam_matrix_prev=CamMatrixPrev;}
   Int  get()C {CamMatrix=cam_matrix; CamMatrixPrev=cam_matrix_prev; return end;}
};
extern Memc<GroupMatrix> SkeletonOpaqueGroups, SkeletonEmissiveGroups, BlendGroups, SkeletonShadowGroups;
...
struct ShaderDraw
{
   Int         first_shader_material; // keep 'first_shader_material' as first member, because it's used most often
   ShaderBase *shader;
...
{
   Int next_shader_material, first_shader_material_mesh; // keep 'next_shader_material' as first member, because it's used most often
...
{
   Int       next_shader_material_mesh; // keep 'next_shader_material_mesh' as first member, because it's used most often
 C MeshPart *mesh;
#if SUPPORT_MATERIAL_CHANGE_IN_RENDERING
...
#if SUPPORT_MESH_STENCIL
   Byte                     stencil_value;
#endif

   OpaqueShaderMaterialMeshInstance& set(Int next_instance);
};
...
struct ShadowShaderMaterialMeshInstance
{
 ...
 C Memc<ShaderParamChange> *shader_param_changes;
   ...
   ShadowShaderMaterialMeshInstance& set(Int next_instance);
};
...
struct SkeletonShaderMaterial
{
 C Material *material; // keep this as first member, because it's used most often
   Int       next_skeleton_shader_material, first_mesh_instance;
...
struct SkeletonShaderMaterialMeshInstance
{
 ...
 C Memc<ShaderParamChange> *shader_param_changes;
   ...
   void set(Int next_instance, C MeshRender &mesh);
   void set(Int next_instance, C MeshPart   &mesh) {set(next_instance, mesh.render);}
};
...
struct SkeletonInstance
{
 C AnimatedSkeleton *anim_skel;
   ...
   Int& newInstance(ShaderBase &shader, C Material &material);
...
struct SkeletonBlendInstance
{
 C AnimatedSkeleton   *anim_skel;
   ...
   void set(C AnimatedSkeleton &anim_skel);
   Int& newInstance(ShaderBase &shader, C Material &material, UInt type);
...
struct BlendInstance
{
   ...
   struct BlendObj // blend object
   {
      BlendObject *blend_obj;
      Int          group;
   };
   ...
   union // Data
   {
      ...
      BlendObj              blend_obj ; // blend object
   ...
   void setZ    (C VecD &pos, Flt offset);
...
struct BlendInstancesClass : Memc<BlendInstance>
{
   ...
   BlendInstance& add(BlendObject        &blend_obj );
