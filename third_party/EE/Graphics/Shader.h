...
struct ShaderParam // Shader Parameter
{
   ...
   T1(TYPE) void set(C TYPE    &data            ) {set((CPtr)&data, SIZE(data));}
            ...
            void fromDivNormalized(C Vec  &v, C Matrix3 &m); // set vector3D value from "v/m"
            void fromDivNormalized(C Vec  &v, C Matrix  &m); // set vector3D value from "v/m"
            void fromDivNormalized(C Vec  &v, C MatrixM &m); // set vector3D value from "v/m"
            void fromDivNormalized(C VecD &v, C Matrix3 &m); // set vector3D value from "v/m"
            void fromDivNormalized(C VecD &v, C Matrix  &m); // set vector3D value from "v/m"
            void fromDivNormalized(C VecD &v, C MatrixM &m); // set vector3D value from "v/m"
...
struct Shader
{
   ...
   void draw(                C Rect *rect, C Rect &uv)C;                            // apply custom 2D effect on the screen, 'image'=image to automatically set as 'Img' shader image, 'rect'=screen rectangle for the effect (set null for full viewport), 'uv'=source image texture coordinates
   void draw(C Image *image, C Rect *rect, C Rect &uv)C;                            // apply custom 2D effect on the screen, 'image'=image to automatically set as 'Img' shader image, 'rect'=screen rectangle for the effect (set null for full viewport), 'uv'=source image texture coordinates
   void draw(C Image &image, C Rect *rect, C Rect &uv)C {draw(&image  , rect, uv);} // apply custom 2D effect on the screen, 'image'=image to automatically set as 'Img' shader image, 'rect'=screen rectangle for the effect (set null for full viewport), 'uv'=source image texture coordinates*/
};
#if EE_PRIVATE
