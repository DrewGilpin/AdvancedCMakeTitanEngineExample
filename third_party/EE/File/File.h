...
struct File
{
...
#endif
   void  close(); // close file, but without releasing helper memory
   ...
   Bool   read   (C SPakFile &file, C SPak   &pak                     ); // try to read Simple Pak   file, writing is not allowed in this mode, false on fail
#if EE_PRIVATE
#if ANDROID
