...
struct Blocks
{
   ...
   // mesh
   Blocks& setMesh(Flt uv_scale, C BlocksOcclusion *occl=null, C BoxI *local_box=null, C VecI2 &blocks_pos=0, C BlocksMap *map=null, C CMemPtr<Light> &lights=null, Bool optimize=false, Flt max_face_length=-1); // set Blocks mesh, 'uv_scale'=texture coordinates scale, 'occl'=Blocks occlusion object used for ambient occlusion calculation (use null for no AO), 'local_box'=if specified then the mesh will only be updated in specified local coordinates (use null for setting entire mesh), 'blocks_pos'=position of this Blocks object inside the 'BlocksMap' (based on this position neighbors will be obtained from the map), 'map'=map of all Blocks objects (you should pass a pointer to your custom class that extends from 'BlocksMap' to provide custom 'findBlocks' method which will be used for finding neighbors to this Blocks object), 'lights'=array of world space lights, 'optimize'=if automatically optimize the mesh by welding coplanar faces which will result in faster rendering ('max_face_length'=max allowed face length, -1=no limit)
...
private:
   ...
   struct Part
   {
      struct Vtx
      {
         ...
         void set(C Vec &pos, C Vec2 &uv, C Color &col) {T.pos=pos; T.uv=uv; T.col=col;}
      };
      Memc<Vtx  > vtxs ;
      Memc<VecI4> quads;
 ...
 C Level*   toLevel (Int y, Int &last)C {return ConstCast(T).toLevel(y, last);}
   ...
   void buildLevel(Flt uv_scale, MemPtrN<Part, 256> parts, Int min_x, Int min_z, Int max_x, Int max_z, Int level_i, C Neighbors &neighbors, LevelBrightness (&lb)[2]);
#endif
};
/******************************************************************************/
